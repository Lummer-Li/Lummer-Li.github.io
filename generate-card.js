// 依赖：需安装 node-fetch（用于请求 GitHub API）
const fs = require('fs');
const fetch = require('node-fetch');

// 配置：替换为你的用户名和 GitHub Token（可选，避免限流）
const USERNAME = 'Lummer-Li';
const GITHUB_TOKEN = process.env.GITHUB_TOKEN; // 从 Actions 环境变量获取
const CURRENT_YEAR = new Date().getFullYear();

// 1. 调用 GitHub API 获取统计数据
...
async function main() {
  try {
    ...
    fs.writeFileSync('stats-card-auto.svg', svg);
    console.log('SVG 卡片生成成功！');
  } catch (err) {
